...
s = ["aa", "bb", "cc"]
b = [True, False, True]
df = pd.DataFrame(list(zip(n, s, b)),
                  columns=['n', 's', 'b'])  # Create a dataframe from the three lists and name its columns
print(df)
...
people = pd.read_csv("./people.txt", sep=";", header=None,
                     names=["Age", "Height", "Weight"])  # Read a csv file with different options
